...
class PathEnumerationList<ID, VALUE> private constructor(
    ...
    companion object {
        ...
        fun <ID, VALUE> fromTreeNode(
            ...
            val list =
                rootTreeNode.fold(emptyList<PathEnumerationListItem<ID, VALUE>>()) { acc, element, currentIndexes ->
                    val path = (0..currentIndexes.size).mapNotNull { level ->
                        rootTreeNode.findSubTreeNodeByIndexes(currentIndexes.take(level))?.let { getNodeId(it.value) }
                    }
                    acc.plus(
...
data class PathEnumerationListItem<ID, VALUE>(
    val nodeId: ID,
    val path: List<ID>, // TODO validation for empty pattern
    val value: VALUE
) {
